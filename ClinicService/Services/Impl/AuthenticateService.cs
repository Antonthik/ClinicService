...
namespace ClinicService.Services.Impl
{
    public class AuthenticateService : IAuthenticateService
    {
        ...
        public SessionContext GetSessionInfo(string sessionToken)//получает токен и получаем текущую сессию(чтобы пропустить авторизацию-не обращаться много раз)
        {
            SessionContext sessionContext;

            lock(_sessions){
                _sessions.TryGetValue(sessionToken, out sessionContext);//активную сессию проверяем - получаем токен если он существует во временном словаре.
            }

            if (sessionContext == null) //если сессия закрыта
            {
                using IServiceScope scope = _serviceScopeFactory.CreateScope();//1
                ClinicServiceDbContext context = scope.ServiceProvider.GetRequiredService<ClinicServiceDbContext>();//2

                AccountSession session = context.AccountSessions.FirstOrDefault(item => item.SessionToken == sessionToken);//3 по токену смотрим в базе
                ...
                sessionContext = GetSessionContext(account, session);//создаем сессию
                
                lock (_sessions)
                {
                    _sessions[sessionContext.SessionToken] = sessionContext;//добавляем сессию в колекцию
                }
